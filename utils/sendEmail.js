...
async function sendCustomsClearedEmail(toEmail, shipment) {
  const { data, error } = await resend.emails.send({
    from: "Fast Way Shiping <delivery@fastwayshiping.com>",
    to: toEmail,
    subject: `Shipment Cleared by Customs — ${shipment.trackingNumber}`,
    html: `
      <div style="font-family:Segoe UI,sans-serif;max-width:600px;margin:0 auto;background:#f0f2f5;padding:24px;border-radius:12px;">

        <div style="background:#1e3a5f;border-radius:10px;padding:24px;text-align:center;margin-bottom:20px;">
          <h1 style="color:#fff;margin:0;font-size:1.4rem;">Fast Way Shiping</h1>
          <p style="color:rgba(255,255,255,0.7);margin:6px 0 0;font-size:0.9rem;">Customs Clearance Update</p>
        </div>
        ...
        <div style="background:#fff;border-radius:10px;padding:24px;margin-bottom:16px;">
          ...
          <table style="width:100%;border-collapse:collapse;font-size:0.85rem;">
            ${shipment.customsRef ? `<tr style="border-bottom:1px solid #f1f5f9;">
              <td style="padding:8px 0;color:#64748b;">Customs Ref #</td>
              <td style="padding:8px 0;color:#1e293b;font-weight:700;text-align:right;">${shipment.customsRef}</td>
            </tr>` : ''}
            ${shipment.dutyAmount ? `<tr style="border-bottom:1px solid #f1f5f9;">
              <td style="padding:8px 0;color:#64748b;">Duty Amount</td>
              <td style="padding:8px 0;color:#1e293b;font-weight:600;text-align:right;">${shipment.dutyAmount}</td>
            </tr>` : ''}
            ${shipment.clearedDate ? `<tr style="border-bottom:1px solid #f1f5f9;">
              <td style="padding:8px 0;color:#64748b;">Cleared Date</td>
        ...
        <p style="text-align:center;font-size:0.75rem;color:#94a3b8;margin:0;">
          © 2026 Fast Way Shipping. All rights reserved.
        </p>
      </div>
    `,
  });
  if (error) {
    console.error("Resend customs cleared error:", error);
    throw new Error(error.message);
  }
